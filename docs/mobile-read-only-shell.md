...
The shared mobile read model can represent projection warnings from stored events, and the smoke snapshot covers that display contract. The runnable JVM dev host starts with built-in public-safe sample state by default and accepts `--config <properties-file>` to render sanitized stored-event state through the same read-only mobile contract.
...
Run it against sanitized stored-event configuration:
