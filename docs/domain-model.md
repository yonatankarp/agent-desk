...
- `app.operator.mobile`: mobile-facing display contract types such as `MobileOperatorStateContract`, derived from the shared operator projection without importing CLI, desktop, runtime adapters, persistence internals, or UI mapping.
...
Runtime observation imports are described in [Runtime adapter boundary](runtime-adapter-boundary.md). The mobile read model is described in [Mobile display-parity contract](mobile-read-only-contract.md).
